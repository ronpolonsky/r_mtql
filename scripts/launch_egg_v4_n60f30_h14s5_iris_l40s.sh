#!/usr/bin/env bash
# Submit a matched Egg-v4 n60f30 H14/S5 trio to normal Iris L40S nodes.
# Transformer/MLP: critic sees all outcomes; actor sees successes only.
# BC: success-only sampling, enforced by m_real.py for the Egg BC agent.
set -euo pipefail
...
[[ -s "${DATASET_CACHE}/metadata.json" ]] || die "Cache metadata is missing"
[[ -s "${NORM_STATS_PATH}/norm_stats.json" ]] || die "Normalization stats are missing"
...
if [[ "${DRY_RUN}" == 1 ]]; then
  echo "Validated all three submissions; no jobs submitted."
  exit 0
fi

job_csv=$(IFS=,; echo "${job_ids[*]}")
echo "Submitted n60f30 H14/S5 trio: ${job_ids[*]}"
