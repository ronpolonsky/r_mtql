...
def get_exp_name(seed, algo=None, hist_length=None, image_obs=False):
    """Return the experiment name."""
    exp_name = ''
    if algo is not None:
        exp_name += f'{algo}_'
    if hist_length is not None:
        exp_name += f'h{hist_length}_'
    if image_obs:
        exp_name += "img_"
...
def setup_wandb(
    ...
    if mode == 'offline':
        if wandb_osh is None or TriggerWandbSyncHook is None:
            raise ImportError(
                "wandb_osh is required when wandb_mode='offline'. "
                "Install wandb_osh or use online/disabled mode."
            )
...
def get_wandb_video(renders=None, n_cols=None, fps=15, frame_skip=1):
    ...
    Args:
        ...
        frame_skip: Keep every N-th frame to reduce video size. 1 = keep all frames.
    """
    # Subsample frames if requested.
    if frame_skip > 1:
        renders = [render[::frame_skip] for render in renders]

    # Pad videos to the same length with black frames.
    max_length = max([len(render) for render in renders])
    for i, render in enumerate(renders):
        assert render.dtype == np.uint8

        pad_length = max_length - len(render)
        if pad_length > 0:
            pad = np.zeros((pad_length, *render.shape[1:]), dtype=np.uint8)
