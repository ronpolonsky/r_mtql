...
#SBATCH --job-name=candy-scoop-bc
...
SAVE_DIR="${SAVE_DIR:-/iris/u/ronpo/mtql-runs/candy_scoop_pixel_cue_bc}"
AGENT_CONFIG="${AGENT_CONFIG:-agents/new_bc_flow_transformer_real.py}"
SEED="${SEED:-0}"
...
if [[ -n "${RESTORE_PATH}" && -z "${RESTORE_EPOCH}" ]] || [[ -z "${RESTORE_PATH}" && -n "${RESTORE_EPOCH}" ]]; then
...
args=(
  ...
  "--project=${PROJECT}"
  "--wandb_run_group=candy-scoop-real-bc"
  "--wandb_mode=online"
  "--enable_wandb=1"
)

if [[ -n "${DATASET_CACHE}" ]]; then args+=("--dataset_cache=${DATASET_CACHE}"); fi

if [[ -n "${CHECKPOINT_DIR}" ]]; then args+=("--checkpoint_dir=${CHECKPOINT_DIR}"); fi
if [[ -n "${RESTORE_PATH}" ]]; then args+=("--restore_path=${RESTORE_PATH}" "--restore_epoch=${RESTORE_EPOCH}"); fi
if [[ "${RESUME}" == "1" ]]; then args+=("--resume"); fi
if [[ "${OVERWRITE}" == "1" ]]; then args+=("--overwrite"); fi
if [[ -n "${CHECKPOINT_KEEP_PERIOD}" ]]; then args+=("--checkpoint_keep_period=${CHECKPOINT_KEEP_PERIOD}"); fi

printf '%s\n' \
  "candy-scoop real BC: project=${PROJECT} agent=${AGENT_CONFIG} cue_mode=${CUE_MODE} cache=${DATASET_CACHE:-none} W&B=online seed=${SEED} train_steps=${TRAIN_STEPS} hist_length=${HIST_LENGTH} hist_stride=${HIST_STRIDE} action_chunk_size=25 p_aug=${P_AUG} n_succ=${N_SUCC} n_fails=${N_FAILS} batch_size=${BATCH_SIZE} flow_steps=10"
cd "${MTQL_ROOT}"
exec "${args[@]}"
