#!/usr/bin/env python3
"""Offline MTQL training on DROID trajectories.

This entry point follows ``m_main.py``'s native MTQL training/checkpoint
loop, while replacing the simulator environment factory with the DROID data
adapter. It supports candy visual/language cues and unconditioned egg data.
"""
...
def _validate_flags() -> None:
    ...
    if FLAGS.agent.get("agent_name") not in {
        ...
        raise ValueError(
            "m_real.py received an unsupported real-DROID agent name."
        )
...
def main(argv):
    ...
    if FLAGS.dataset_kind == "egg":
        ...
        if config.get("agent_name") == "new_bc_flow_transformer_real":
            train_dataset.set_sampling_idxs(success_idxs)
            sampling_policy = "success_only"
            sampling_transition_count = egg_success_transition_count
            print(
                "[m_real] egg BC sampling: success-only "
                f"({egg_success_episode_count} episodes, "
                f"{egg_success_transition_count} transitions); excluded "
                f"{egg_failure_episode_count} failed episodes and "
                f"{egg_failure_transition_count} failure transitions",
                flush=True,
            )
        else:
            print(
                "[m_real] egg RL sampling: all outcomes "
                f"({egg_success_episode_count} successes + "
                f"{egg_failure_episode_count} failures, "
                f"{sampling_transition_count} transitions)",
                flush=True,
            )
    ...
    if restore_step is not None:
        ...
        if (
            restored_sampling_policy is not None
            and restored_sampling_policy != sampling_policy
        ):
            raise ValueError(
        ...
        if (
            FLAGS.dataset_kind == "egg"
            and sampling_policy == "success_only"
            and restored_sampling_policy is None
        ):
            raise ValueError(
                "Refusing to resume a legacy egg BC checkpoint without a "
                "success-only sampling contract. Start corrected BC from "
                "step 0; legacy checkpoints were trained on failures too."
            )
        if metadata:
    ...
    try:
        for step in tqdm.tqdm(
            ...
            if FLAGS.p_aug > 0.0 and np.random.rand() < FLAGS.p_aug:
                batch = augment_droid_batch(
                    batch,
                    jax.random.PRNGKey(FLAGS.seed + step),
                )
            ...
            if FLAGS.save_augmented_video and not saved_augmented_video:
                _save_augmented_training_sample(
                    batch, cue_targets, episode_success, FLAGS.save_augmented_video
                )
                saved_augmented_video = True

            start = time.time()
            agent, update_info = agent.update(batch, step=step)
