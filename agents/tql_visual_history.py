"""TQL with visual observations and history for the critic and actor."""
import copy
...
class ViTPatchImageEncoder(nn.Module):
    ...
    def __call__(self, x, train: bool = True):
        ...
        gh, gw, c = x.shape[-3:]
        return jnp.reshape(x, (*x.shape[:-3], gh * gw, c))
...
def _flatten_history(history_observations, history_actions):
    batch_shape = history_observations.shape[:-2]
    hist_obs_flat = history_observations.reshape(*batch_shape, -1)
    hist_act_flat = history_actions.reshape(*batch_shape, -1)
    return jnp.concatenate([hist_obs_flat, hist_act_flat], axis=-1)


class HistoryActorVectorField(nn.Module):
    hidden_dims: Any
    action_dim: int
    ...
    def __call__(
        ...
        if not is_encoded and self.encoder is not None:
            ...
            if history_observations is not None:
                history_observations = self.encoder(history_observations)
        if history_observations is not None and history_actions is not None:
            hist_flat = _flatten_history(history_observations, history_actions)
            observations = jnp.concatenate([hist_flat, observations], axis=-1)
        return self.mlp(observations, actions, times, is_encoded=True)
...
class TransformerValue(nn.Module):
    ...
    def __call__(
        ...
        action_proj = nn.Dense(self.hidden_dim, name='action_projection')
        action_tokens = action_proj(actions[..., None])

        if observations.ndim >= 3:
            batch_shape = observations.shape[:-2]
            n_state = observations.shape[-2]
            state_proj = nn.Dense(self.hidden_dim, name='vit_to_hidden')
            obs_tokens = state_proj(observations)
        else:
            batch_shape = observations.shape[:-1]
            n_state = observations.shape[-1]
            state_proj = nn.Dense(self.hidden_dim, name='obs_projection')
            obs_tokens = state_proj(observations[..., None])
        ...
        token_list = []
        if history_observations is not None and history_actions is not None:
            hist_length = history_observations.shape[-3] if history_observations.ndim >= 4 else history_observations.shape[-2]
            for k in range(hist_length):
                h_obs = history_observations[..., k, :, :] if history_observations.ndim >= 4 else history_observations[..., k, :]
                h_act = history_actions[..., k, :]
                if observations.ndim >= 3:
                    h_obs_tokens = state_proj(h_obs)
                else:
                    h_obs_tokens = state_proj(h_obs[..., None])
                h_act_tokens = action_proj(h_act[..., None])
                if self.use_modality_embeddings:
                    h_obs_tokens = h_obs_tokens + state_type_embed_broadcast
                    h_act_tokens = h_act_tokens + action_type_embed_broadcast
                token_list.extend([h_obs_tokens, h_act_tokens])
        token_list.extend([obs_tokens, action_tokens])
...
class TQLVisualHistoryAgent(flax.struct.PyTreeNode):
    """FQL with visual history for transformer critic and flow matching actors."""

    rng: Any
    network: Any
    config: Any = nonpytree_field()

    def _shift_history_for_next_state(self, batch):
        history_observations = batch.get('history_observations')
        history_actions = batch.get('history_actions')
        if history_observations is None or history_actions is None:
            return None, None
        next_history_observations = jnp.concatenate(
            [history_observations[:, 1:], batch['observations'][:, None]],
            axis=1,
        )
        next_history_actions = jnp.concatenate(
            [history_actions[:, 1:], batch['actions'][:, None]],
            axis=1,
        )
        return next_history_observations, next_history_actions
    ...
    def critic_loss(self, batch, grad_params, rng, step=None):
        ...
        next_history_observations, next_history_actions = self._shift_history_for_next_state(batch)
        next_actions = self.sample_actions(
            batch['next_observations'],
            history_observations=next_history_observations,
    ...
    @jax.jit
    def _sample_actions_batch(self, observations, action_seed, history_observations=None, history_actions=None):
        ...
        if 'implicit' in self.config["extraction"]:
            ...
            n_history_actions = (
                jnp.repeat(jnp.expand_dims(history_actions, 1), self.config['num_samples'], axis=1)
                if history_actions is not None else None
            )
            for i in range(self.config['flow_steps']):
                ...
                vels = self.network.select('actor_bc_flow')(
                    ...
                    actions,
                    t,
                    history_observations=n_history_observations,
                    history_actions=n_history_actions,
                )
    ...
    def sample_actions(
        ...
        if observations.ndim == 1:
            ...
            if history_observations is not None:
                history_observations = history_observations[None]
            if history_actions is not None:
                history_actions = history_actions[None]
            squeeze = True
    ...
    @jax.jit
    def compute_flow_actions(self, observations, noises, history_observations=None, history_actions=None):
        """Compute actions from the BC flow model using the Euler method."""
        actions = noises
        # Euler method
        batch_shape = _policy_noise_batch_shape(observations)
        for i in range(self.config['flow_steps']):
    ...
    def create(cls, seed, example_batch, config):
        ...
        ex_history_observations = example_batch.get('history_observations')
        ex_history_actions = example_batch.get('history_actions')
        ...
        actor_bc_flow_def = HistoryActorVectorField(
            hidden_dims=config['actor_hidden_dims'],
            action_dim=action_dim,
            layer_norm=config['actor_layer_norm'],
            encoder=encoders['actor_bc_flow'],
        )
        actor_onestep_flow_def = HistoryActorVectorField(
            hidden_dims=config['actor_hidden_dims'],
            action_dim=action_dim,
            layer_norm=config['actor_layer_norm'],
        ...
        network_info = dict(
            critic=(critic_def, (ex_observations, ex_actions, ex_history_observations, ex_history_actions)),
            target_critic=(copy.deepcopy(critic_def), (ex_observations, ex_actions, ex_history_observations, ex_history_actions)),
            actor_bc_flow=(actor_bc_flow_def, (ex_observations, ex_actions, ex_times, ex_history_observations, ex_history_actions)),
            actor_onestep_flow=(actor_onestep_flow_def, (ex_observations, ex_actions, None, ex_history_observations, ex_history_actions)),
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            dropout_rate=0.0,
            num_ensembles=2,
            use_state_action_separation=False,
